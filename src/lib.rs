...
pub fn run(
    opts: Opt, 
    cb: &dyn Fn(Vec<u8>, &Aes128Gcm) -> Result<Vec<u8>, Box<dyn error::Error>> 
    ...
    if opts.infile.is_dir(){
        ...
        for entry in WalkDir::new(opts.infile).into_iter().filter_map(|e| e.ok()).filter(|e| e.path().is_file()) {
            ...
            fs::write(entry.path(), cb(fs::read(entry.path())?, &cipher)?)?
        }
    } else {
        fs::write(opts.infile.clone(), cb(fs::read(opts.infile)?, &cipher)?)?
    };
...
/// Encryption callback
pub fn encrypt_cb(contents: Vec<u8>, cipher: &Aes128Gcm) -> Result<Vec<u8>, Box<dyn error::Error>> {
